...
Pipeline health uses explicit states: `HEALTHY`, `STALE`, `FAILING`, `DISABLED`, `NOT_APPLICABLE`, and `INSUFFICIENT_HISTORY`. It is not an opaque score. System separates source health (whether governed data is available and fresh) from automation health (whether live executors can satisfy durable data obligations). An enabled schedule creates preferred execution times; an obligation records what data window must actually be completed. Late catch-up is reported as recovered late, not on time, and a technically successful process does not satisfy an obligation while provider data remains pending. Disabled schedules never create new obligations.
