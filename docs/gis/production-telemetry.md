...
The worker long-polls up to ten messages, validates the envelope again, resolves the site, runs canonical telemetry validation, and deletes only after database commit. Each new logical queue batch has one worker-owned database transaction containing its ingestion run, session/event/calculator/conversion changes, final counters, and transport-batch row. The domain service stages changes without committing underneath that transaction. If canonical validation rejects even one event, or any operation fails before commit, the entire batch rolls back: no canonical rows, `RUNNING` ingestion run, or transport accounting remain. The message stays available for retry and eventual DLQ routing after five receives. Empty polls are normal, and stop signals finish the current bounded poll.

Idempotency is layered. The stable public `batch_id` is unique within site scope and is the primary logical-delivery key; `(transport, transport_message_id)` remains unique for operational transport identity; canonical `event_id` and lifecycle keys remain unique within tenant/site scope. Database constraints are the final race-safe authority. A duplicate logical batch under a new SQS message ID finds the already committed transport row, skips ingestion, and deletes the duplicate message without creating another ingestion run or accounting row. If the database commits but SQS deletion fails, redelivery follows that same path and safely retries deletion. Ordering is not assumed; canonical timestamps and session `last_event_at` tolerate late events. Lifecycle events that refer to a not-yet-seen calculator run remain retryable failures until the start event arrives or the message reaches the DLQ.

Every successfully committed new logical batch creates one `ingestion_run` with source connection, source acquisition method, rights policy, schema/collector version, batch/trace/message identifiers and counters. Events link to that run. Failed transactional attempts rely on SQS/DLQ durability rather than an out-of-band database commit. `telemetry_transport_batch` therefore represents successful logical batches only and accounts for bytes, accepted/rejected events and event-level duplicates by site/period. Existing first-party rights remain `UNKNOWN` until explicitly reviewed. Existing dbt lineage from session/event/calculator/conversion raw assets to marts remains unchanged.
