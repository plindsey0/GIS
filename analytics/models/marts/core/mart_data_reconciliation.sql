...
select k.*, coalesce(g.gsc_clicks, 0) as gsc_clicks,
  ...
  g.tenant_id is not null as gsc_data_present, a.tenant_id is not null as ga4_data_present,
  f.tenant_id is not null as first_party_data_present,
  case
    when g.tenant_id is null and a.tenant_id is null and f.tenant_id is null then 'NO_TRAFFIC'
    when g.tenant_id is null or a.tenant_id is null or f.tenant_id is null then 'PARTIAL_SOURCE_COVERAGE'
