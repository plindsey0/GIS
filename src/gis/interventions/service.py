...
VERSION = "INTERVENTION_CONTRACT_V2"

TYPES: dict[str, dict[str, Any]] = {
    "UPDATE_CONTENT_ASSET": {
        "family": InterventionFamily.CONTENT,
        "entities": ["URL"],
        "parameters": ["target_url", "content_scope"],
        "metrics": ["GSC_CLICKS", "GSC_CTR", "GA4_SESSIONS"],
        "execution_mode": "MANUAL",
    },
    "CHANGE_PAGE_METADATA": {
        "family": InterventionFamily.SEO,
        "entities": ["URL"],
        "parameters": ["target_url", "metadata_scope"],
        "metrics": ["GSC_CLICKS", "GSC_CTR", "GSC_POSITION"],
        "execution_mode": "MANUAL",
    },
    "IMPROVE_PAGE_EXPERIENCE": {
        "family": InterventionFamily.EXPERIENCE,
        "entities": ["URL"],
        "parameters": ["target_url", "experience_scope"],
        "metrics": ["CRUX_LCP", "CRUX_INP", "CRUX_CLS"],
        "execution_mode": "MANUAL",
    },
    "CHANGE_CTA": {
        "family": InterventionFamily.CONVERSION,
        "entities": ["URL"],
        "parameters": ["target_url", "cta_identifier"],
        "metrics": ["TELEMETRY_CTA_INTERACTION", "TELEMETRY_CONVERSION"],
        "execution_mode": "MANUAL",
    },
    "EXPAND_COLLECTION": {
        "family": InterventionFamily.COLLECTION,
        "entities": ["QUERY", "TOPIC", "URL", "DOMAIN", "MARKET", "MARKET_SEGMENT"],
        "parameters": ["requested_evidence_type"],
...
METRICS: dict[str, tuple[str, str, str]] = {
    ...
    # CrUX contracts must never silently consume Lighthouse lab observations.
    "CRUX_LCP": ("CrUX LCP", "pagespeed_field", "milliseconds"),
    "CRUX_INP": ("CrUX INP", "pagespeed_field", "milliseconds"),
    "CRUX_CLS": ("CrUX CLS", "pagespeed_field", "score"),
...
class InterventionService:
    ...
    def ensure_registry(
        self,
    ) -> tuple[dict[str, InterventionTypeDefinition], dict[str, MetricDefinition]]:
        types: dict[str, InterventionTypeDefinition] = {}
        metrics: dict[str, MetricDefinition] = {}
        for key, spec in TYPES.items():
            type_row = self.session.scalar(
                select(InterventionTypeDefinition).where(
                    InterventionTypeDefinition.key == key,
                    InterventionTypeDefinition.version == VERSION,
                )
            )
            if not type_row:
                type_row = InterventionTypeDefinition(
                    key=key,
                    version=VERSION,
                    name=key.replace("_", " ").title(),
                    description=f"Structured {key.casefold().replace('_', ' ')} contract.",
                    family=spec["family"],
                    execution_mode=spec["execution_mode"],
                    autonomy_level="HUMAN_APPROVAL_REQUIRED",
                    enabled=True,
                    schema_json={
                        "applicable_entities": spec["entities"],
                        "required_parameters": spec["parameters"],
        ...
        for key, (name, source, unit) in METRICS.items():
            metric_row = self.session.scalar(
                select(MetricDefinition).where(
                    MetricDefinition.key == key, MetricDefinition.version == VERSION
                )
            )
            if not metric_row:
                metric_row = MetricDefinition(
                    key=key,
                    version=VERSION,
                    name=name,
                    source_system=source,
                    unit=unit,
                    grain="ANALYTICAL_ENTITY_PERIOD",
                    enabled=True,
                )
    ...
    def ensure_lineage(self) -> None:
        ...
        execution = register_asset(
            self.session, "gis_core.intervention_execution", AssetType.EVIDENCE, AssetLayer.CORE
        )
        outcome = register_asset(
            self.session, "gis_core.intervention_outcome", AssetType.EVIDENCE, AssetLayer.CORE
        )
    ...
    def valid_types(self, opportunity: Opportunity) -> list[dict[str, Any]]:
        ...
        return [
            {"key": key, **spec}
            for key, spec in TYPES.items()
            if entity.entity_type.value in spec["entities"]
        ]
    ...
    def validate(
        ...
        errors.extend(
            f"MISSING_PARAMETER:{key}" for key in spec["parameters"] if not parameters.get(key)
        )
        ...
        *,
        ...
        identity = digest(
            {
                ...
                "type": type_key,
                "version": VERSION,
                "entity": opportunity.analytical_entity_id,
                "parameters": parameters,
            }
        )
        existing = self.session.scalar(
            select(Intervention).where(Intervention.identity_hash == identity)
        )
        if existing:
            return existing
        row = Intervention(
            ...
            intervention_type_id=types[type_key].id,
            ...
            title=f"{types[type_key].name}: {opportunity.title}",
        ...
        self.session.add(
            InterventionHypothesis(
                ...
                target_entity_id=row.analytical_entity_id,
                rationale=rationale,
            )
        )
        contract = MeasurementContract(
            intervention_id=row.id,
            version=VERSION,
            baseline_strategy="FIXED_PRE_PERIOD",
            baseline_start=baseline_start,
            baseline_end=baseline_end,
            measurement_start=measurement_start,
            measurement_end=measurement_end,
            washout_days=max(0, (measurement_start - baseline_end).days),
            comparison_method="BEFORE_AFTER",
            minimum_evidence=DemandEvidenceStrength.SUPPORTED,
            freshness_days=14,
            exclusions_json=[],
        ...
        self.session.add(
            MeasurementMetric(
                measurement_contract_id=contract.id,
        ...
        self.session.add(
            InterventionLifecycleEvent(
                intervention_id=row.id,
                to_status=InterventionStatus.DRAFT,
                actor=proposed_by,
                reason="structured intervention created",
    ...
    def baseline(self, intervention_id: uuid.UUID) -> dict[str, Any]:
        ...
        contract = self.session.scalar(
            select(MeasurementContract)
            .where(MeasurementContract.intervention_id == row.id)
            .order_by(MeasurementContract.created_at.desc())
        )
        return {
            "intervention_id": row.id,
