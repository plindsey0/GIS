...
class RecommendationService:
    ...
    def ensure_policy(self) -> RecommendationPolicy:
        row = self.session.scalar(
            select(RecommendationPolicy).where(
                RecommendationPolicy.key == "DEFAULT_GOVERNED",
                RecommendationPolicy.version == POLICY_VERSION,
            )
        )
        if not row:
            row = RecommendationPolicy(
                key="DEFAULT_GOVERNED",
                ...
                policy_json={
                    ...
                    "watching_intelligence_gap_only": True,
                    "minimum_sufficiency": "SUPPORTED",
                    "maximum_candidates": 3,
                    "maximum_repairs": 1,
                    "external_ai_requires_explicit_inference_rights": True,
                    "blocked_candidates": False,
                    "ranking": "ORDINAL_CONTRACT_FIT_V1",
    ...
    def ensure_lineage(self) -> None:
        ...
        run = register_asset(
            self.session, "gis_core.recommendation_run", AssetType.EVIDENCE, AssetLayer.CORE
        )
        recommendation = register_asset(
            self.session, "gis_core.recommendation", AssetType.TABLE, AssetLayer.CORE
        )
        intervention = register_asset(
            self.session, "gis_core.intervention", AssetType.TABLE, AssetLayer.CORE
        )
        ...
        register_lineage(
            self.session, recommendation, intervention, reference="accepted candidate only"
        )
    ...
    def eligibility(self, opportunity: Opportunity) -> tuple[bool, list[str], list[Any]]:
        ...
        if opportunity.status is not OpportunityStatus.ACTIVE:
            if not (
                opportunity.status is OpportunityStatus.WATCHING
                and opportunity.family is OpportunityFamily.INTELLIGENCE_GAP
            ):
        ...
        for package in packages:
            if package.sufficiency not in {
                DemandEvidenceStrength.SUPPORTED,
                DemandEvidenceStrength.STRONGLY_SUPPORTED,
            }:
    ...
    def context(self, opportunity: Opportunity, packages: list[Any]) -> dict[str, Any]:
        ...
        return {
            ...
            "evidence_packages": [
                {
                    "id": str(p.id),
                    "sufficiency": p.sufficiency.value,
                    "corroboration": p.corroboration.value,
                    "source_independence": p.source_independence.value,
                    "rights_usability": p.rights_usability.value,
                    "conflict_count": p.conflict_count,
                }
                for p in packages[:25]
            ],
            "applicable_intervention_types": [
                {
                    "key": item["key"],
            ...
            "limitations": list(opportunity.limitations_json),
            "untrusted_source_text_is_data_only": True,
            "tools_available": [],
        }

    def validate_output(
        self, opportunity: Opportunity, context: dict[str, Any], output: dict[str, Any]
    ) -> list[str]:
        errors: list[str] = []
        if not isinstance(output.get("summary"), str) or not isinstance(
            output.get("candidates"), list
        ):
        ...
        for candidate in output["candidates"]:
            ...
            if any(
                key not in candidate.get("parameters", {}) for key in spec["required_parameters"]
            ):
    ...
    def generate(
        self, opportunity_id: uuid.UUID, *, dry_run: bool = False, force: bool = False
    ) -> dict[str, Any]:
        ...
        context = (
            self.context(opportunity, packages)
            if packages
            else {"applicable_intervention_types": []}
        )
        if dry_run or not eligible or not context["applicable_intervention_types"]:
            return {
                "status": "DRY_RUN" if dry_run else "NO_VALID_RECOMMENDATION",
                "eligible": eligible,
                "blockers": blockers
                or (
                    ["NO_APPLICABLE_INTERVENTION"]
                    if not context["applicable_intervention_types"]
                    else []
                ),
                "applicable_intervention_types": [
                    item["key"] for item in context["applicable_intervention_types"]
                ],
                "provider": self.provider.key,
                "model": self.provider.model_identifier,
                "ai_call_would_occur": eligible and bool(context["applicable_intervention_types"]),
                "ai_calls": 0,
                "estimated_cost": 0 if self.provider.key == "fixture" else None,
            }
        context_hash = digest(
            {
                "opportunity": opportunity.identity_hash,
                "packages": [p.identity_hash for p in packages],
        ...
        existing = self.session.scalar(
            select(RecommendationRun).where(RecommendationRun.context_hash == context_hash)
        )
        if existing and not force:
            recommendation = self.session.scalar(
                select(Recommendation).where(Recommendation.run_id == existing.id)
            )
            return {
                "status": "REUSED",
                "run_id": existing.id,
        ...
        if force:
            context_hash = digest(
                {"base": context_hash, "regenerated_at": datetime.now(timezone.utc)}
            )
        run = RecommendationRun(
            tenant_id=opportunity.tenant_id,
            site_id=opportunity.site_id,
            opportunity_id=opportunity.id,
            recommendation_policy_id=policy.id,
            status=RecommendationRunStatus.RUNNING,
            provider_key=self.provider.key,
            model_identifier=self.provider.model_identifier,
            model_configuration_json={"temperature": 0, "structured_output": True, "tools": []},
            prompt_version=PROMPT_VERSION,
        ...
        recommendation = Recommendation(
            ...
            market_definition_id=opportunity.market_definition_id,
            market_definition_version=opportunity.market_definition_version,
            status=RecommendationStatus.READY_FOR_REVIEW,
            summary=output["summary"],
            assumptions_json=[],
            limitations_json=list(opportunity.limitations_json),
            identity_hash=digest({"run": run.id, "opportunity": opportunity.id}),
        )
        ...
        for rank, item in enumerate(output["candidates"], 1):
            self.session.add(
                RecommendationCandidate(
                    ...
                    intervention_type_id=types[item["intervention_type"]].id,
                    rank=rank,
                    fit=item["fit"],
                    validation_state=CandidateValidationState.VALID,
                    parameters_json=item["parameters"],
                    target_metric_key=item["target_metric"],
                    expected_direction=ExpectedDirection(item["expected_direction"]),
                    rationale=item["rationale"],
                    assumptions_json=item["assumptions"],
                    limitations_json=[*opportunity.limitations_json, *item["limitations"]],
                    feasibility=FeasibilityState.UNKNOWN,
                    measurement_readiness=MeasurementReadiness.PARTIAL,
                    validation_errors_json=[],
                )
            )
        for package in packages:
    ...
    def review(
        ...
        *,
        ...
        if (
            decision
            in {RecommendationReviewDecision.ACCEPT, RecommendationReviewDecision.PARTIAL_ACCEPT}
            and not candidates
        ):
        ...
        self.session.add(
            RecommendationReview(
                ...
                accepted_candidate_ids_json=[str(c.id) for c in candidates],
                reviewed_at=datetime.now(timezone.utc),
            )
        )
        if decision in {
            RecommendationReviewDecision.ACCEPT,
            RecommendationReviewDecision.PARTIAL_ACCEPT,
        }:
            opportunity = self.session.get(Opportunity, recommendation.opportunity_id)
            assert opportunity
            now = datetime.now(timezone.utc)
            for candidate in candidates:
                type_row = self.session.get(
                    InterventionTypeDefinition, candidate.intervention_type_id
                )
                assert type_row
                intervention = self.interventions.create(
                    ...
                    candidate.rationale,
                    proposed_by=f"recommendation:{recommendation.id}",
                )
            ...
            recommendation.status = (
                ...
                if decision is RecommendationReviewDecision.ACCEPT
                else RecommendationStatus.PARTIALLY_ACCEPTED
            )
    ...
    def list(self, tenant_id: uuid.UUID, site_id: uuid.UUID) -> list[Recommendation]:
        return list(
            self.session.scalars(
                ...
                .where(Recommendation.tenant_id == tenant_id, Recommendation.site_id == site_id)
