...
def package(session: Session, sufficiency: DemandEvidenceStrength, *, rights: RightsUsability = RightsUsability.USABLE, conflicts: int = 0, visibility: str = "LOW") -> tuple[Tenant, Site, EvidencePackage]:
    ...
    entity = service.entity(tenant.id, site.id, AnalyticalEntityType.QUERY, "va loan trend", metadata={"owned_visibility": visibility})
    contract = EvidenceContract(contract_key=f"TEST_{uuid.uuid4().hex}", contract_version="1", description="test", requirements_json={}, active=True)
    session.add(contract)
    session.flush()
    quality_run = EvidenceQualityRun(tenant_id=tenant.id, site_id=site.id, method_version="1", assessed_at=NOW, fingerprint=uuid.uuid4().hex, input_count=1, package_count=1, metadata_json={})
    session.add(quality_run)
    session.flush()
    row = EvidencePackage(quality_run_id=quality_run.id, tenant_id=tenant.id, site_id=site.id, analytical_entity_id=entity.id, evidence_contract_id=contract.id, condition_key="demand", classification="EMERGING", period_start=date(2026, 8, 1), period_end=date(2026, 8, 31), sufficiency=sufficiency, identity_resolution="EXACT", source_independence=SourceIndependenceState.INDEPENDENT, corroboration=CorroborationState.CORROBORATED, rights_usability=rights, conflict_count=conflicts, independent_source_count=2, limitations_json=[], identity_hash=uuid.uuid4().hex, method_version="1")
    session.add(row)
    session.flush()
...
def test_registry_version_and_sparse_behavior(session: Session) -> None:
    ...
    assert set(DETECTORS) == {"EMERGING_DEMAND_VISIBILITY_GAP", "DEMAND_ACCELERATION_GAP", "HIGH_VALUE_EVIDENCE_GAP"}
...
def test_limited_rights_conflict_and_visibility_gate(session: Session) -> None:
    tenant, site, _ = package(session, DemandEvidenceStrength.LIMITED, rights=RightsUsability.BLOCKED, conflicts=1)
...
def test_dismiss_restore_preserves_computed_state(session: Session) -> None:
    ...
    assert row.status is OpportunityStatus.DISMISSED and row.computed_status is OpportunityStatus.ACTIVE
...
def test_cli_registry_is_json_ready() -> None:
    ...
    assert all("key" in detector and "materiality_components" in detector for detector in payload["detectors"])
