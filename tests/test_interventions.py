...
from gis.interventions.service import METRICS, VERSION, InterventionService
...
def test_crux_contracts_are_field_only() -> None:
    assert VERSION == "INTERVENTION_CONTRACT_V2"
    assert {METRICS[key][1] for key in ("CRUX_LCP", "CRUX_INP", "CRUX_CLS")} == {"pagespeed_field"}


def opportunity(
    session: Session, entity_type: AnalyticalEntityType = AnalyticalEntityType.URL
) -> Opportunity:
    ...
    entity = EvidenceQualityService(session).entity(
        ...
        site.id,
        entity_type,
        "https://vahomemath.test/loan" if entity_type is AnalyticalEntityType.URL else "va loan",
    )
    ...
    row = Opportunity(
        ...
        opportunity_type="EMERGING_DEMAND_VISIBILITY_GAP",
        status=OpportunityStatus.ACTIVE,
        computed_status=OpportunityStatus.ACTIVE,
        priority=OpportunityPriority.HIGH,
        evidence_sufficiency=DemandEvidenceStrength.SUPPORTED,
        title="Supported demand condition",
        condition_description="Supported condition, not a recommendation.",
        detected_at=NOW,
        period_start=NOW.date(),
        period_end=NOW.date(),
        identity_hash=digest({"test": entity.id}),
        materiality_json={},
...
def create(
    session: Session, opp: Opportunity | None = None
) -> tuple[InterventionService, Opportunity, Intervention]:
    opp = opp or opportunity(session)
    service = InterventionService(session)
    row = service.create(
        opp.id,
        "UPDATE_CONTENT_ASSET",
        {"target_url": "https://vahomemath.test/loan", "content_scope": "factual sections"},
        "GSC_CLICKS",
        ...
        "The update is hypothesized to increase clicks because the linked opportunity remains active.",
    )
