...
def test_unchanged_recollection_is_audited_and_changes_preserve_history(session: Session) -> None:
    ...
    details = list(session.scalars(select(OwnedSurfaceObservationDetail).order_by(
        OwnedSurfaceObservationDetail.created_at
    )))
    assert len(details) == 2
    assert details[1].previous_observation_id == details[0].observation_id
    assert "CONTENT" in details[1].change_classification
