...
def test_fixture_builds_governed_snapshot_packet_and_workbench(session: Session) -> None:
    ...
    evidence_item = session.scalar(select(EvidencePackageItem).where(
        EvidencePackageItem.evidence_package_id == evidence.id
    ))
    assert evidence_item and evidence_item.metadata_json == {
...
def test_duplicate_unchanged_changed_and_comparison_history(session: Session) -> None:
    ...
    assert details[1].previous_observation_id == details[0].observation_id
    assert details[2].change_classification == "CHANGED"
    assert details[2].previous_observation_id == details[1].observation_id
