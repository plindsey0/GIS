...
type ExplorerProps = {title: string; description: string; endpoint: string; empty: string; columns: Column[]; filters?: Filter[]};

function ExplorerInner({title, description, endpoint, empty, columns, filters = []}: ExplorerProps) {
  ...
  return <><PageHeader eyebrow="Intelligence explorer" title={title} description={description}/><div className="filterBar"><label>Search<input aria-label="Search" value={query.get("search") ?? ""} onChange={(event) => update("search", event.target.value)} placeholder="Search meaningful identity…"/></label>{filters.map((filter) => <label key={filter.name}>{filter.label}<select value={query.get(filter.name) ?? ""} onChange={(event) => update(filter.name, event.target.value)}><option value="">All</option>{filter.options.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select></label>)}<label>Page size<select value={limit} onChange={(event) => update("limit", event.target.value)}><option>25</option><option>50</option><option>100</option></select></label></div>{error ? <ErrorState message={error}/> : !data ? <LoadingState/> : data.items.length === 0 ? <EmptyState title={empty} detail="The current filters may exclude available governed records. Missing does not mean zero."/> : <><div className="tableWrap"><table><thead><tr>{columns.map((column) => <th key={column.key}>{column.label}</th>)}</tr></thead><tbody>{data.items.map((item) => <tr key={String(item.id)}>{columns.map((column) => { const value = item[column.key]; const display = column.format === "date" ? formatDate(value) : column.format === "count" ? formatNumber(value) : column.format === "decimal" ? formatNumber(value, "decimal") : column.format === "currency" ? formatNumber(value, "currency") : column.format === "list" && Array.isArray(value) ? value.join(", ") || "None recorded" : String(value ?? "Unknown"); return <td key={column.key}>{column.key === "label" && item.href ? <Link href={String(item.href)}>{display}</Link> : column.key === "status" ? <StatusBadge>{humanize(display)}</StatusBadge> : display}</td>; })}</tr>)}</tbody></table></div><nav className="pagination" aria-label="Pagination"><p>Showing {from}–{to} of {data.total}</p><button disabled={page <= 1} onClick={() => update("page", String(page - 1))}>Previous</button><span>Page {page} of {Math.max(1, Math.ceil(data.total / limit))}</span><button disabled={to >= data.total} onClick={() => update("page", String(page + 1))}>Next</button></nav></>}</>;
}
