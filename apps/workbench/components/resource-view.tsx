...
export function ResourceList({kind}: {kind: keyof typeof copy}) {
  ...
  return <><PageHeader eyebrow="Intelligence workbench" title={labels.title} description={labels.description}/>{error ? <ErrorState message={error}/> : !data ? <LoadingState/> : data.items.length === 0 ? <EmptyState title={labels.empty} detail="GIS preserves missing and insufficient states; this does not mean the underlying condition is zero or resolved."/> : <div className="tableWrap"><table><thead><tr><th>Resource</th><th>Status</th><th>Updated</th></tr></thead><tbody>{data.items.map((item) => <tr key={String(item.id)}><td>{String(item.name ?? item.title ?? item.id)}</td><td><StatusBadge>{String(item.status ?? item.state ?? "Unknown")}</StatusBadge></td><td>{String(item.updated_at ?? item.created_at ?? "—")}</td></tr>)}</tbody></table></div>}</>;
}
