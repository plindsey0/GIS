...
type Capability = {
  ...
  choices:Array<{id:string;label:string;type:string}>; cadence:string;
...
export function ProviderConfigurationPage({providerKey}:{providerKey:string}) {
  ...
  const load=useCallback(async()=>{
    try {
      ...
      setData(result);setPolicy(result.policy);setCaps(result.capabilities);
  ...
  const payload=(activate=false)=>({policy:{...policy,actor:"workbench-admin",reason:"Reviewed provider configuration"},capabilities:caps,activate});
  ...
  return <>
    ...
    {step===null?<>
      <section className="providerSection"><h2>Operational status</h2><dl className="detailGrid"><div><dt>Implementation</dt><dd>{humanize(d.implementation_status)}</dd></div><div><dt>Last successful collection</dt><dd>{formatDate(d.last_collection)}</dd></div><div><dt>Next scheduled</dt><dd>{formatDate(d.next_collection)}</dd></div><div><dt>Collection readiness</dt><dd>{d.blocking_reason?humanize(d.blocking_reason):"Active policy"}</dd></div></dl></section>
      <section className="providerSection"><h2>Configuration summary</h2><div className="providerGrid">{caps.map(c=><article className="recordCard providerCapability" key={c.key}><StatusBadge>{c.enabled?"Enabled":"Disabled"}</StatusBadge><h3>{c.name}</h3><p>{c.description}</p><p>{c.target_ids.length} authorized targets · {humanize(c.cadence)}</p><p>Freshness: {c.freshness_hours} hours</p>{d.is_commercial&&<p>Unit price: {c.unit_price===null?"Unknown":`$${c.unit_price}`} · {humanize(c.pricing_provenance)}</p>}</article>)}</div></section>
      <section className="providerSection"><h2>Execution policy</h2>{data.schedules.length===0?<p>No derived execution schedule yet. Configure collection to bind targets and cadence.</p>:data.schedules.map(s=><p key={s.id}>{humanize(s.status)} · {s.cron} ({s.timezone}) · Next: {formatDate(s.next_at)}</p>)}</section>
      {d.is_commercial&&<section className="providerSection"><h2>Budget and request limits</h2><div className="providerFields"><div><p>Monthly soft: {policy.monthly_soft_budget===null||policy.monthly_soft_budget===undefined?"Not configured":`$${policy.monthly_soft_budget}`}</p><p>Monthly hard: {policy.monthly_hard_budget===null||policy.monthly_hard_budget===undefined?"Not configured":`$${policy.monthly_hard_budget}`}</p><p>Per-target execution hard: {policy.per_run_hard_budget===null||policy.per_run_hard_budget===undefined?"Not configured":`$${policy.per_run_hard_budget}`}</p></div><div><p>Daily requests: {policy.daily_request_limit??"Not configured"}</p><p>Monthly requests: {policy.monthly_request_limit??"Not configured"}</p><p>Per-execution requests: {policy.per_run_request_limit??"Not configured"}</p><p>Unknown cost: {policy.allow_unknown_cost?"Explicitly permitted within request limits":"Blocked"}</p></div></div></section>}
      <section className="providerSection"><h2>Usage</h2><p>Today: ${d.budget.spent_day} · This month: ${d.budget.spent_month}</p>{d.usage.length===0?<p>No control-plane usage recorded. Historical ingestion is not an inferred cost ledger.</p>:<ul>{d.usage.map(u=><li key={u.id}>{formatDate(u.occurred_at)} · {u.requests} requests · {humanize(u.status)} · Actual cost: {u.actual_cost===null?"Unknown":`$${u.actual_cost}`}</li>)}</ul>}</section>
      ...
      {step===2&&<><p>Only selected canonical targets are authorized. Discovery does not authorize spending. Maximum 100 targets per capability.</p><label>Find targets<input type="search" value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search query or domain"/></label>{caps.filter(c=>c.enabled).map(c=><fieldset key={c.key}><legend>{c.name} — {c.target_ids.length} selected</legend><div className="providerTargets">{c.choices.filter(t=>t.label.toLowerCase().includes(search.toLowerCase())).map(t=><label key={t.id}><input type="checkbox" checked={c.target_ids.includes(t.id)} onChange={e=>updateCap(c.key,{target_ids:e.target.checked?[...c.target_ids,t.id]:c.target_ids.filter(id=>id!==t.id)})}/>{t.label}</label>)}</div>{c.choices.length===0&&<p>No canonical targets are available for this capability.</p>}<button className="secondaryButton" onClick={()=>updateCap(c.key,{target_ids:[]})}>Clear selection</button></fieldset>)}</>}
